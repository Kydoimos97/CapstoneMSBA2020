...
```{r, fig.height = 10, fig.width = 7, fig.cap='Relationships between Target and Numeric Variables'}
num_list <- colnames(df[sapply(df, is.factor)])
plot_list <- list(1)
...
for (i in num_list) {
  ...
  counter = counter+1
  x = ggplot(data=df, aes_string(x = "sales", y = i)) +
  geom_boxplot() + ggtitle(paste0("sales vs ", as.character(i), sep= " ")) +
    theme_fivethirtyeight() + theme(plot.title = element_text(size=10)) 
