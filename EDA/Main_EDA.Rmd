...
```{r}
download.file("https://github.com/Kydoimos97/CapstoneMSBA2020/raw/main/Data/CapstoneProjectInfoRevised.rds", destfile = "CapstoneProjectInfoRevised.rds")
download.file("https://github.com/Kydoimos97/CapstoneMSBA2020/raw/main/Data/CapstoneProjectProducts.rds", destfile= "CapstoneProjectProducts.rds")
...
As shown the deployed strategies elimited all occurences of moderate to strong multicollinearity. only weak multicollinearity remains allowing algoritmhs to perform most optimally.
...
for (i in num_list) {
  ...
  counter = counter+1
  x = ggplot(data=na.omit(df[df$sales < 150,]), aes_string(x = "sales", y = i)) +
  geom_point() + geom_smooth(method = lm, color="red", size=0.5) + ggtitle(paste0("sales vs ", as.character(i), sep= " ")) + xlim(0,150) +
...
grid.arrange(grobs=plot_list, widths = c(1,1), ncol=nCol, layout_matrix =  rbind(c(1,2),
                                                                                 c(3,4), 
                                                                                 c(5,6)), 
             top = "Relationships between Target and Numeric Variables")

#rm(plot_list, counter, num_list, x, i,, nCol)
```

```{r, fig.height = 10, fig.width = 7, fig.cap='Relationships between Target and Numeric Variables'}
fac_list <- colnames(df[sapply(df, is.factor)])
fac_list <- fac_list[-c(1,2,3)]
plot_list <- list(1)
...
for (i in fac_list) {
  ...
  counter = counter+1
  x = ggplot(data=na.omit(df[df$sales < 150,]), aes_string(y = "sales", x = i)) +
  geom_boxplot() + ggtitle(paste0("sales vs ", as.character(i), sep= " ")) + ylim(0,150) + coord_flip() + 
    theme_fivethirtyeight() + theme(plot.title = element_text(size=10)) 
...
grid.arrange(grobs=plot_list, widths = c(1,1), ncol=nCol, layout_matrix =  rbind(c(1,2),
                                                                                 c(3,4)), 
             top = "Relationships between Target and Factor Variables")

#rm(plot_list, counter, fac_list, x, i, n, nCol)
```

```{r}
ggplot(na.omit(df[df$sales < 150,]), aes(x=date, y=sales)) + 
  geom_line(size = 0.5) aaa
```aaaa
