...
def getInput():
    ...
    th_std = np.std(x.Sales_Difference) * int(std_dev_cl.get()) + np.mean(
        x.Sales_Difference
    )
    temp = x
                        ...
                        for __q in __q_values:
                            ...
                            try:
                                ...
                                tk.messagebox.showwarning(
                                    title="Outcome", message="ARIMA FAILED"
                                )
                                break
                if __best_cfg == (0, 0, 0):
                    __best_cfg = __order
...
# Explanation
logo = tk.PhotoImage(
    file="C:/Users/wille/Documents/GitHub/CapstoneMSBA2020/Resources/logo_maverick.gif",
    master=app,
)
...
w2 = tk.Label(app, justify=tk.LEFT, padx=20, text=explanation).grid(
...
gridsearch_d_inp.insert(END, "Yes")
